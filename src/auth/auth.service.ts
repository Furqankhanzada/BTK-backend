...
export class AuthService {
    ...
    async forgotPassword(forgotPasswordDto: ForgotPasswordDto): Promise<void> {
        const user = await this.usersService.findOneByEmailOrNumber(forgotPasswordDto.emailOrNumber)
        ...
        const hashSixDigitRandomCode = await bcrypt.hash(`${ user.phone }-${ sixDigitRandomCode }`, 10);
        ...
        return await this.emailService.sendRawEmail({
    ...
    async verifyCode(verifyCodeDto: VerifyCodeDto): Promise<any> {
        ...
        const user = await this.usersService.findOneByEmailOrNumber(verifyCodeDto.emailOrNumber)
        if (!user) {
            throw new BadRequestException('Invalid User')
        }
        ...
        if(!user?.verification?.code){
            throw new BadRequestException('Looks like verification code has expired.')
        }
        ...
        const payload = { emailOrNumber: verifyCodeDto.emailOrNumber, sub: user._id };
        return {
            access_token: this.jwtService.sign(payload),
        };
    }
